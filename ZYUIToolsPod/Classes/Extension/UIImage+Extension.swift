...
public extension UIImage {
    ...
    static func gradientImage(_ gradientType: GradientType = .leftToRight,
                              fromColor: UIColor,
                              toColor: UIColor,
                              size: CGSize,
                              cornerRadius: CGFloat,
                              corners: UIRectCorner) -> UIImage? {
        ...
        guard let gradient = CGGradient(colorsSpace: context.colorSpace, colors: colorArr, locations: locations) else {
            return nil
        }
        
        let pointTuples = gradientType.locationTuples(size)
        context.setShouldAntialias(true)
        let path = UIBezierPath(roundedRect: CGRect(x: 0, y: 0, width: size.width, height: size.height), byRoundingCorners: corners, cornerRadii: CGSize(width: cornerRadius, height: cornerRadius))
        context.addPath(path.cgPath)
        context.clip()
        context.drawLinearGradient(gradient, start: pointTuples.start, end: pointTuples.end, options: [.drawsBeforeStartLocation, .drawsAfterEndLocation])
...
public enum GradientType {
    ...
    var locationTuples: (CGSize) -> (start: CGPoint, end: CGPoint) {
        
        return { (size) -> (start: CGPoint, end: CGPoint)  in
            
            switch self {
            case .leftToRight:
                
                return (CGPoint(x: 0, y: size.height/2.0), CGPoint(x: size.width, y: size.height/2.0))
            ...
            case .topToBottom:
                ...
                return (CGPoint(x: size.width/2.0, y: 0), CGPoint(x: size.width/2.0, y: size.height))
            }
